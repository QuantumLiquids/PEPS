...
void TestSingleModeNonDBMarkovChainDistribution(
  ...
  for (size_t i = 0; i < weights.size(); ++i) {
    EXPECT_NEAR(state_distribution[i] * weight_sum, weights[i], 1e-4);
  }
}

TEST(NonDBMCMCTest, SingleModeMarkovChainDistribution) {
  TestSingleModeNonDBMarkovChainDistribution({0.4, 0.35, 0.25}, 100000000);
  TestSingleModeNonDBMarkovChainDistribution({0.01, 0.1, 0.6, 0.29}, 100000000);
  TestSingleModeNonDBMarkovChainDistribution({1, 0.5, 0.3, 0.01, 0.06, 2}, 100000000);
}
...
//  TestSingleModeNonDBMarkovChainDistribution({1, 0.3, 1e-30}, 1000000, 2);
  double temperature = std::log(1 + std::sqrt((double) 2));
  double boltz_w = std::exp(2.0 / temperature);
  TestSingleModeNonDBMarkovChainDistribution({boltz_w, boltz_w, 1}, 1000000, 0);
}
...
///< https://arxiv.org/pdf/hep-lat/9301017.pdf, it's an approximation for thermal dynamic limit so it may not good to used as a benchmark here.
...
//  EXPECT_NEAR(energy0, energy1, en_err1 + en_err0);
