...
* 
* Pre-computed exact energies (from tools/calculate_exact_energies.cpp):
* Standard q=3 Potts model, T_critical = 1/ln(1+sqrt(3)) = 1.00505254
* 3x3, T=0.7*T_c (0.70353678): E_exact = 0.32711538
* 3x3, T=1.5*T_c (1.50757881): E_exact = 7.16804143
* 4x4, T=T_c (1.00505254): E_exact = 4.88542773
* 
* ConservedPottsModel has different critical temperature
* due to particle number conservation constraint.
*/
...
TEST(SuwaTodoUpdateTest, BasicFunctionality) {
  ...
  // Test 2: Zero weight states should never be selected
  {
  ...
  // Test 3: Deterministic case - only one non-zero weight
  {
...
class PottsModel {
  public:
    ...
    double CalculateExactEnergy() {
      ...
      for (size_t i = 0; i < total_num_configurations; ++i) {
        //fix last site spin = 0  by Zq symmetry
  ...
  private:
    ...
    double CalculateEnergy_() const {
      ...
      // Optimize energy calculation - avoid function calls in inner loop
      for (size_t x = 0; x < size_; ++x) {
        for (size_t y = 0; y < size_; ++y) {
          size_t idx = x * size_ + y;
          size_t spin = spins_[idx];
          size_t sum_neighbors = (spins_[((x + 1) % size_) * size_ + y] == spin) +
              (spins_[x * size_ + (y + 1) % size_] == spin);
          energy -= sum_neighbors;
        }
      }
      return energy;
...
TEST(PottsModelTest, MonteCarloSimulationTest) {
  const size_t size = 3; // 3x3 system
  const size_t q = 3;
  // Critical temperature for q-state Potts model: T_c = 1/ln(1 + sqrt(q))
  const double T_critical = std::log(1 + std::sqrt((double) q));
  const double temperature = 0.7 * T_critical; // Near critical point

  // Moderate sample sizes for good statistics near critical point
  const size_t thermalization_steps = 10000;
  const size_t sampling_steps = 50000;

  PottsModel model(size, q, temperature);

  // Use pre-computed exact energy (from tools/calculate_exact_energies.cpp)
  // 3x3, T=0.7*T_c: exact energy = 0.32711538
  const double energy_ex = 0.32711538;

  Timer monte_carlo_timer("monte carlo");
  for (size_t i = 0; i < thermalization_steps; ++i) {
    model.MonteCarloSweep(); //warm up
  }

  for (size_t i = 0; i < sampling_steps; ++i) {
    model.MonteCarloSweep();
    model.MonteCarloSample();
  }
  monte_carlo_timer.PrintElapsed();

  // Near critical point, allow for larger fluctuations
  double mc_error = model.GetEnergyErr(30);
  double tolerance = 3.0 * mc_error;
  EXPECT_NEAR(model.GetEnergy(), energy_ex, tolerance);
}
    ...
    void MonteCarloSweep3() {
      ...
      for (size_t i = 0; i < 2 * size_ * size_; ++i) {
        ...
        if (dis(gen) < 0.5) {
          ...
          if (col < size_ - 2) {
            // Ensure we don't go out of bounds for horizontal
          ...
          // Ensure we don't go out of bounds for vertical
...
TEST(ConservedPottsModelTest, MonteCarloSimulationTest) {
  const size_t size = 3; // 3x3 system
  const size_t q = 3;
  // Note: ConservedPottsModel (with particle number conservation) has DIFFERENT
  // critical temperature than standard Potts model. The standard formula
  // T_c = 1/ln(1 + sqrt(q)) applies only to non-conserved models.
  const double T_critical_standard = std::log(1 + std::sqrt((double) q));
  const double temperature = 2.0 * T_critical_standard;

  // Moderate sampling for unit testing - balance between speed and accuracy
  const size_t thermalization_steps = 5000;
  const size_t sampling_steps = 25000;
  ...
  double en_err1 = model.GetEnergyErr(25);
  ...
  double en_err2 = model.GetEnergyErr(25);

  // Algorithms should agree within statistical error
  double tolerance1 = 3.0 * (en_err1 + en_err2);
  EXPECT_NEAR(energy1, energy2, tolerance1);
  ...
  double en_err3 = model.GetEnergyErr(25);

  double tolerance2 = 3.0 * (en_err1 + en_err3);
  EXPECT_NEAR(energy1, energy3, tolerance2);
}
