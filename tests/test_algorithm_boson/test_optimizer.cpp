...
class OptimizerTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    // Set up test parameters with more lenient convergence criteria for testing
  ...
  std::tuple<TenElemT, SITPST, double> MockEnergyEvaluator(const SITPST &state) {
    ...
    SITPST gradient = state; //Mock derivative. It is gradient of |state|^2 (element wise).
...
TEST_F(OptimizerTest, LineSearchOptimization) {
  ...
  test_params_ =
      OptimizerParams(OptimizerParams::CoreParams(1000, 1e-6, 1e-6, 20, {0.1, 0.05, 0.01}), GradientLineSearch);
...
TEST_F(OptimizerTest, DifferentOptimizationSchemes) {
  ...
  for (auto scheme : schemes) {
    test_params_ = OptimizerParams(OptimizerParams::CoreParams(1000, 1e-6, 1e-6, 20, {0.1}), scheme);
...
TEST_F(OptimizerTest, AdvancedStopGradientConvergence) {
  ...
  test_params_ = OptimizerParams(OptimizerParams::CoreParams(1000, 1e-6, 1e-10, 20, {0.1, 0.05, 0.01}), StochasticGradient);
...
TEST_F(OptimizerTest, AdvancedStopEnergyConvergence) {
  ...
  test_params_ = OptimizerParams(OptimizerParams::CoreParams(1000, 1e-10, 1e-6, 20, {0.1, 0.05, 0.01}), StochasticGradient);
...
TEST_F(OptimizerTest, AdvancedStopPlateauDetection) {
  ...
  test_params_ = OptimizerParams(OptimizerParams::CoreParams(1000, 1e-6, 1e-6, 2, {0.1, 0.05, 0.01}), StochasticGradient);
...
TEST_F(OptimizerTest, ShouldStopMethod) {
  test_params_ = OptimizerParams::CreateStochasticGradient({0.1}, 1000);
  OptimizerT optimizer(test_params_, comm_, rank_, mpi_size_);
...
TEST_F(OptimizerTest, NoEarlyStop) {
  // Set very high tolerances and patience to prevent early stopping
  test_params_ = OptimizerParams(OptimizerParams::CoreParams(1000, 1e-20, 1e-20, 1000, {0.1, 0.05, 0.01}), StochasticGradient);
  ...
  EXPECT_TRUE(result.converged);
  // Should run for max_iterations since no stopping criteria are met
