...
* This file targets Adam optimizer on four small models.
*/
...
struct Z2SpinlessFreeFermionAdamTools : public MPITest {
...
TEST_F(Z2SpinlessFreeFermionAdamTools, ExactSumGradientOptWithAdam) {
  ...
  for (size_t i = 0; i < t2_list.size(); i++) {
    ...
    qlpeps::OptimizerParams::BaseParams base_params(300, 1e-15, 1e-15, 100, 0.01);
    qlpeps::AdamParams adam_params(0.9, 0.999, 1e-8, 0.0);
    qlpeps::OptimizerParams opt_params(base_params, adam_params);

    std::string test_name = std::string("Adam_SpinlessFreeFermion_t2=") + std::to_string(t2);
    RunPureOptimizerAdam<Model, TenElemT, QNT, SITPST>(
      model, split_index_tps, all_configs, trun_para, Ly, Lx, energy_exact, opt_params, test_name, this->comm, this->rank, this->mpi_size);
  }
}

// ===== Heisenberg (Trivial) =====
struct TrivialHeisenbergAdamTools : public MPITest {
...
TEST_F(TrivialHeisenbergAdamTools, ExactSumGradientOptWithAdam) {
  ...
  // Adam with default parameters
  qlpeps::OptimizerParams::BaseParams base_params(500, 1e-15, 1e-30, 200, 0.05);
  qlpeps::AdamParams adam_params(0.9, 0.999, 1e-8, 0.0);
  qlpeps::OptimizerParams opt_params(base_params, adam_params);

  std::string test_name = "Adam_Heisenberg_Trivial";
  RunPureOptimizerAdam<Model, TenElemT, QNT, SITPST>(
    model, split_index_tps, all_configs, trun_para, Ly, Lx, energy_exact, opt_params, test_name, this->comm, this->rank, this->mpi_size);
}

// ===== Transverse Ising (Trivial) =====
struct TrivialTransverseIsingAdamTools : public MPITest {
  using IndexT = Index<TrivialRepQN>;
  using QNSctT = QNSector<TrivialRepQN>;
  using DTensor = QLTensor<TEN_ELEM_TYPE, TrivialRepQN>;
...
TEST_F(TrivialTransverseIsingAdamTools, ExactSumGradientOptWithAdam) {
  ...
  // Adam with default parameters
  qlpeps::OptimizerParams::BaseParams base_params(150, 1e-15, 1e-30, 50, 0.02);
  qlpeps::AdamParams adam_params(0.9, 0.999, 1e-8, 0.0);
  qlpeps::OptimizerParams opt_params(base_params, adam_params);

  std::string test_name = "Adam_TransverseIsing_Trivial";
  RunPureOptimizerAdam<Model, TenElemT, QNT, SITPST>(
    model, split_index_tps, all_configs, trun_para, Ly, Lx, energy_exact, opt_params, test_name, this->comm, this->rank, this->mpi_size);
}

// ===== t-J model (Z2) =====
struct Z2tJAdamTools : public MPITest {
  using IndexT = Index<fZ2QN>;
  using QNSctT = QNSector<fZ2QN>;
  using DTensor = QLTensor<TEN_ELEM_TYPE, fZ2QN>;
...
TEST_F(Z2tJAdamTools, ExactSumGradientOptWithAdam) {
  ...
  // Adam with default parameters - adjusted for t-J model convergence
  qlpeps::OptimizerParams::BaseParams base_params(600, 1e-15, 1e-30, 300, 0.01);
  qlpeps::AdamParams adam_params(0.9, 0.999, 1e-8, 0.0);
  qlpeps::OptimizerParams opt_params(base_params, adam_params);

  std::string test_name = "Adam_tJ_Model";
  RunPureOptimizerAdam<Model, TenElemT, QNT, SITPST>(
    model, split_index_tps, all_configs, trun_para, Ly, Lx, energy_exact, opt_params, test_name, this->comm, this->rank, this->mpi_size);
}
