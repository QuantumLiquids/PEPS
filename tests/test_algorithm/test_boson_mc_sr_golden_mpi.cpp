...
*
* Description: MPI golden regression test for bosonic MC gradient + SR natural gradient.
*   Exercises the multi-rank CG solver path (mean_dot_v broadcast in SRSMatrix).
...
class CachedHeisenbergSolver {
 public:
  using SITPST = SplitIndexTPS<TenElemT, QNT>;
  using WaveFuncComp = TPSWaveFunctionComponent<TenElemT, QNT>;
  using HoleTN = TensorNetwork2D<TenElemT, QNT>;

  explicit CachedHeisenbergSolver(double jz, double jxy, double pinning00 = 0.0)
      : base_(jz, jxy, pinning00) {}
 ...
 private:
  ...
  SquareSpinOneHalfXXZModelOBC base_;
...
void PrintCurrentValues(const char *label,
  ...
  std::cout << "[GOLDEN-CANDIDATE][" << label << "] "
            << "energy=(" << std::setprecision(17) << std::real(energy) << "," << std::imag(energy) << ") "
            << "grad_norm=" << grad_norm << " "
...
struct BosonMCSRGoldenMPITest : MPITest {
  ...
  SITPST BuildRandomTrivialOBCSITPS() const {
    ...
    for (size_t row = 0; row < kLy; ++row) {
      for (size_t col = 0; col < kLx; ++col) {
        IndexT left = (col == 0) ? IndexT({QNSctT(QNT(), 1)}, TenIndexDirType::IN)
                                 : IndexT({QNSctT(QNT(), 2)}, TenIndexDirType::IN);
        IndexT down = (row == kLy - 1) ? IndexT({QNSctT(QNT(), 1)}, TenIndexDirType::OUT)
                                       : IndexT({QNSctT(QNT(), 2)}, TenIndexDirType::OUT);
        IndexT right = (col == kLx - 1) ? IndexT({QNSctT(QNT(), 1)}, TenIndexDirType::OUT)
                                        : IndexT({QNSctT(QNT(), 2)}, TenIndexDirType::OUT);
        IndexT up = (row == 0) ? IndexT({QNSctT(QNT(), 1)}, TenIndexDirType::IN)
                               : IndexT({QNSctT(QNT(), 2)}, TenIndexDirType::IN);
        for (size_t phys = 0; phys < 2; ++phys) {
          Tensor ten({left, down, right, up});
          ten.Random(QNT());
...
TEST_F(BosonMCSRGoldenMPITest, MCEnergyGradientAndSRGoldenMPI) {
  if (mpi_size != 2) {
    GTEST_SKIP() << "Requires exactly 2 MPI ranks.";
  }

  SITPST sitps = BuildRandomTrivialOBCSITPS();
  CachedHeisenbergSolver<TenElemT, QNT> cached_model(/*jz=*/1.0, /*jxy=*/1.0);
  ...
  MCEnergyGradEvaluator<TenElemT, QNT, CachedMCUpdateSquareNNExchange<TenElemT, QNT>,
                        CachedHeisenbergSolver<TenElemT, QNT>>
  ...
  ASSERT_EQ(eval_res.Ostar_samples.size(), kSamples / mpi_size);

  // All ranks participate in CG solve (exercises MPI broadcast in SRSMatrix).
  ...
  if (kPrintGolden) {
    PrintCurrentValues("boson_mc+sr_mpi",
                       eval_res.energy,
                       grad_norm,
                       grad_probe,
                       grad_probe_rand1,
  ...
  // Golden values captured from trusted code (old SRSMatrix) with mpirun -np 2.
  if constexpr (std::is_same_v<TenElemT, QLTEN_Double>) {
    constexpr double kEnergyReal = 0.27556244501976079;
    constexpr double kEnergyImag = 0.0;
    constexpr double kGradNorm = 0.74374707470451529;
    constexpr double kGradProbeReal = 0.18457380558881978;
    constexpr double kGradProbeImag = 0.0;
    constexpr double kGradProbeRand1Real = 0.3758141797326589;
    constexpr double kGradProbeRand1Imag = 0.0;
    constexpr double kGradProbeRand2Real = 0.50868071499824852;
    constexpr double kGradProbeRand2Imag = 0.0;
    constexpr double kNatGradNorm = 0.17183942705834773;
    constexpr double kNatGradProbeReal = 0.042251265425905828;
    constexpr double kNatGradProbeImag = 0.0;
    constexpr double kNatGradProbeRand1Real = -0.084501379894493578;
    constexpr double kNatGradProbeRand1Imag = 0.0;
    constexpr double kNatGradProbeRand2Real = -0.16409224241209097;
    ...
    constexpr double kEnergyReal = 1.0811466918606778;
    constexpr double kEnergyImag = 0.052571395952151212;
    constexpr double kGradNorm = 4.0665840410291203;
    constexpr double kGradProbeReal = 1.7133573425500987;
    constexpr double kGradProbeImag = 0.039281552415938101;
    constexpr double kGradProbeRand1Real = 2.214399998022611;
    constexpr double kGradProbeRand1Imag = 0.61622020312216474;
    constexpr double kGradProbeRand2Real = 2.2130235007560266;
    constexpr double kGradProbeRand2Imag = 1.5586504176674369;
    constexpr double kNatGradNorm = 0.83990946944095901;
    constexpr double kNatGradProbeReal = 0.35429250570106519;
    constexpr double kNatGradProbeImag = 0.0081225771629004403;
    constexpr double kNatGradProbeRand1Real = 0.46877131328094734;
    constexpr double kNatGradProbeRand1Imag = 0.16454061271256415;
    constexpr double kNatGradProbeRand2Real = 1.3916385014473924;
    constexpr double kNatGradProbeRand2Imag = 0.72611315053019476;
    constexpr size_t kCGIters = 2;
    EXPECT_NEAR(std::real(eval_res.energy), kEnergyReal, 1e-9);
