  ...
  ObservableMap<TenElemT> EvaluateObservables(
    ...
    for (size_t row = 0; row < ly; ++row) {
      for (size_t col = 0; col < lx; ++col) {
        ...
        TenElemT eb_h = EvaluateBondEnergy_<TenElemT, QNT>(
            site1, site_right, config, *sitps, contractor, inv_psi);
        if (e_h.size() != 0) {
          e_h(site1.row(), site1.col()) = eb_h;
        }
        energy += eb_h;

        const SiteIdx site_down{(row + 1) % ly, col};
        TenElemT eb_v = EvaluateBondEnergy_<TenElemT, QNT>(
            site1, site_down, config, *sitps, contractor, inv_psi);
        if (e_v.size() != 0) {
          e_v(site1.row(), site1.col()) = eb_v;
        }
        energy += eb_v;
      }
    }
