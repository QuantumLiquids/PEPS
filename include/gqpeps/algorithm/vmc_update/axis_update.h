...
*
* Description: GraceQ/VMC-PEPS project. Implementation for the axis update.
*/
...
GQTensor<TenElemT, QNT> GrowPhyIndexFromSplitIndexTensorsWithouQN_(
    const std::vector<GQTensor<TenElemT, QNT> *> &split_index_tens
) {
  ...
  GQTensor<TenElemT, QNT> res(whole_indexes);
  res({0, 0, 0, 0, 0}) = 1.0;
