...
本示例基于方格晶格最近邻更新框架，最常见的使用场景是 **OBC + BMPS 收缩**。

如果你要跑 **PBC + TRG**，请确保：

- 你的 `SplitIndexTPS` 是周期边界（PBC），并且
- 你的 `PEPSParams` 中持有 TRG 截断参数，

否则公共一键接口（`VmcOptimize` / `MonteCarloMeasure`）会直接抛出异常以避免“跑错后端”。
...
该函数由基类 `MCUpdateSquareNNUpdateBaseOBC<Derived>` 在横向-纵向顺序扫描过程中多次调用。你只需定义“如何在一个 NN 键上从 (config1, config2) 提议并接受一个新配置”的局部逻辑。

说明：

- OBC/BMPS 使用 `MCUpdateSquareNNUpdateBaseOBC<Derived>`。
- PBC/TRG 也有对应的基类 `MCUpdateSquareNNUpdateBasePBC<Derived>`，其 sweep 风格不同（随机选键更新）。
