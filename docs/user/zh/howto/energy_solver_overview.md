...
在公共一键接口（`VmcOptimize` / `MonteCarloMeasure`）中，`SplitIndexTPS` 的边界条件会与 `PEPSParams` 进行交叉检查，不匹配时会直接报错。
...
- 数学与约定（复数梯度）：`../explanation/model_energy_solver_math.md`
