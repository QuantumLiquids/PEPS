...
- `SGDParams` exposes `weight_decay` as a separate (decoupled) L2 regularization term, applied in an AdamW-style way:
  - first scale parameters by \((1-\eta\lambda)\),
  - then apply the gradient-based update.
- This is intentionally separate from learning-rate schedulers (the scheduler produces \(\eta\); regularization is part of the update rule).

Note:

- This implementation does not expose a separate “dampening” parameter; use learning-rate scheduling instead.
...
auto scheduler = std::make_unique<CosineAnnealingLR>(
  0.01,   // eta_max
  1000,   // T_max (period, in steps)
  0.0     // eta_min
);
```

**Use case**: smooth annealing without manual step boundaries; useful when you want a “slow cool-down” schedule.
...
## Heuristic comparison (not a benchmark)

This table is a *rule-of-thumb* summary for typical VMC-PEPS settings. It is **not** a measured benchmark.

| Algorithm | Convergence speed | Memory use | Numerical stability | Typical use case |
|---|---:|---:|---:|---|
| Stochastic Reconfiguration (SR) | high | medium | high | high-precision physics runs |
| Adam | medium-high | medium-high | medium | fast prototyping / exploration |
| SGD (+ momentum) | medium | low | high | stable production runs |
| AdaGrad | low-medium | medium | medium | sparse-ish gradients / early exploration |
| L-BFGS | high (when it works) | low | low-medium | low-noise / smoother objectives |
