...
|-------------|---------------------|----------------------|-----------------------|
| `TransverseFieldIsingSquareOBC` (and `TransverseFieldIsingSquarePBC`) | `energy`, `spin_z`, `sigma_x`, `SzSz_row` | `energy`: scalar; site observables `{Ly,Lx}` with `{y,x}`; `SzSz_row` flat | Gap resolved—metadata now matches returned keys. |
